...
class FlatService {
  ...
  Future<Response> getSingleFlat(
    ...
    await collectionRef.doc(flatName).get().then((snapshot) {
      Flat flat = FlatExt.fromJson(snapshot.data() as Map<String, dynamic>);
      response.code = 200;
      response.body = 'ok';
  ...
  Stream<List<Flat>> flatsStream({
    ...
    return _db
      ...
      return snapshot.docs.map((doc) {
        return FlatExt.fromJson(doc.data());
      }).toList();
    });
  }
  ...
  Stream<Flat> selectedFlatSteram({
    ...
    return _db
        ...
        .map(
          (snapshot) =>
              FlatExt.fromJson(snapshot.data() as Map<String, dynamic>),
        );
  }
  ...
  Future<Response> getAllFlatsForVm({required String homeId}) async {
    ...
    try {
      ...
      final flatList = flatsSnapshot.docs.map((flat) {
        return FlatExt.fromJson(flat.data() as Map<String, dynamic>);
      }).toList();
